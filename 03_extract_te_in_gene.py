#!/usr/bin/env python3
"""Extract TE annotations overlapping gene regions (gene body ± flanks).
...
def load_gene_bed(filename: str) -> dict[str, list[tuple[int, int, str]]]:
    """Load gene BED: chrom -> list of (start, end, gene_id)."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        required=True,
        help="Output BED with gene_id appended as last column.",
    )
        ...
        for line in fin:
            ...
            for gene_start, gene_end, gene_id in genes[chrom]:
                ...
                if gene_end < te_start:
                    continue
                fout.write(line.rstrip() + "\t" + gene_id + "\n")
